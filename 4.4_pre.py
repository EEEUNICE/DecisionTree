...
def testing_feature(feature, data_train, data_test, labels):
	...
	train_data = [example[feature_index] for example in data_train]
	test_data = [(example[feature_index], example[-1]) for example in data_test] 
	all_feature =set(train_data)
	error = 0.0
	for value in all_feature:
		class_feature = [class_list[i] for i in range(len(class_list)) if train_data[i] == value]
		major = vote(class_feature)
...
def createTree(dataset, features, data_full, features_full, data_test):
	...
	if testing_feature(feature_best, dataset, data_test, features_copy) < testingMajor(vote(classlist), data_test):
		mytree = {feature_best:{}}
	else:
		# total_err += testingMajor(vote(classlist), data_test)
		return vote(classlist)
...
if __name__=='__main__':
	...
	myTree = createTree(data, features, data_full, features_full, data_test)
	print (myTree)
